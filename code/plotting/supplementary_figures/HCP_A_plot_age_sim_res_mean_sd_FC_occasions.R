...
f = 'ridgeCV_zscore_averaged-source_Schaefer400x17_WM+CSF+GS_r2z2r_rest2_hcpaging_695_zscored-beh_interview_age_wnoise_all.csv'
d_age_rest2 = read_csv(paste0(plt_in,f))
...
df = cbind(df_m,df_sd$test_R2)
colnames(df) = c('reliability', 'R2_mean', 'R2_sd')
df$variable = 'rest day 1'

df_sum = df


# age rest day 2
df_m = d_age_rest2 %>% group_by(reliability) %>%
  summarise_at(vars(metrics[c(-1,-2,-13,-14)]), list(name = mean))
colnames(df_m)[2:11] <- metrics[c(-1,-2,-13,-14)]

df_sd = d_age_rest2 %>% group_by(reliability) %>%
  summarise_at(vars(metrics[c(-1,-2,-13,-14)]), list(name = sd))
colnames(df_sd)[2:11] <- metrics[c(-1,-2,-13,-14)]
...
df = cbind(df_m,df_sd$test_R2)
colnames(df) = c('reliability', 'R2_mean', 'R2_sd')
df$variable = 'rest day 2'   

df_sum = rbind(df_sum, df)

df_sum = df_sum[df_sum$variable %in% c('rest day 1', 'rest day 2'), ]
...
ggsave(paste0(plt_out,'rel_squared_Age_R2_mean_sd_wnoise_rel_',rel_max,'_to_',rel_min,'_FC_occasions.png'), pltm, width=4, height=3)
