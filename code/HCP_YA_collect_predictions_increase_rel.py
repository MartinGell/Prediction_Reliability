...
if pipe == 'ridgeCV_zscore_stratified_KFold_confound_removal_wcategorical':
    ...
    beh_file = 'HCP_YA_beh_all_increase_rel'
    pipe = 'ridgeCV_zscore_stratified_KFold'
    first = 'WM_Task_Acc'
    # First load empirical results, then append all simulation res to it
    # ridgeCV_averaged-source_Schaefer400x17_WM+CSF+GS_hcpaging_695-beh_interview_age_interview_age-rseed_123456-cv_res.csv
    res = pd.read_csv(f'{in_path}/pipe_{pipe}{opts}{beh_file}_WM_Task_Acc-rseed_123456-cv_res.csv')
    res['beh'] = first
    behs = pd.read_table('/data/project/impulsivity/prediction_simulations/code/opts/HCP_YA_behs2predict_increase_rel.txt', header=None)
